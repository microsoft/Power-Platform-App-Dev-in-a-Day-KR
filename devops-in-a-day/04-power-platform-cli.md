...
1. `➕ 권한 추가` 버튼을 클릭한 후 `Microsoft API` 탭에서 `Power Automate`를 선택합니다. 이후 `위임된 권한` 메뉴를 선택하고 `Flows.Manage.All` 항목을 체크한 후 `권한 추가` 버튼을 클릭합니다.
...
1. `➕ 권한 추가` 버튼을 클릭한 후 `내 조직에서 사용하는 API` 탭에서  `PowerApps-Advisor`를 선택합니다. 이후 `위임된 권한` 메뉴를 선택하고 `Analysis.All (아니오)` 항목을 체크한 후 `권한 추가` 버튼을 클릭합니다.

## 3. 파워 플랫폼 로그인

1. 자신의 환경에 해당하는 URL을 찾습니다.

   - 파워 오토메이트 화면의 주소를 복사합니다. 주소는 아래와 같은 형식입니다. 자신의 `Participant {{숫자}} Environment` 환경과 `Global Power Platform Bootcamp (default)` 환경의 환경 ID를 찾습니다.

     ```text
     https://make.powerautomate.com/environments/{{환경 ID}}/home
     ```
   ...
   - 환경 URL 링크를 마우스 우클릭으로 복사합니다. URL은 아래와 같은 형식입니다.

     ```text
     https://org{{조직 ID}}.crm{{숫자}}.dynamics.com/
     ```

1. 파워 플랫폼 관리자 화면 상단 중앙의 `⚙️ 설정` 버튼을 클릭한 후 `사용자 + 권한` 👉 `응용 프로그램 사용자` 메뉴를 선택합니다.

   > **NOTE**: 관리자 화면 우측 상단의 `⚙️` 버튼이 아닙니다!

1. `➕ 새 앱 사용자` 버튼을 클릭하여 아래 내용을 입력하고 `만들기` 버튼을 클릭합니다.

   - 앱: `➕ 앱 추가` 버튼 클릭 후 `GPPB Participant {{숫자}}` 선택. 자신의 애플리케이션(클라이언트) ID에 해당하는 것을 선택하세요.
   - 사업부: 드롭다운 메뉴에서 선택
   - 보안 역할: 연필 버튼 클릭 후 `시스템 관리자` 선택

1. GitHub 코드스페이스에서 새 터미널을 열고 아래 명령어를 실행하여 파워 플랫폼 CLI 설치 여부를 확인합니다.
...
1. 아래 명령어를 통해 파워 플랫폼에 로그인합니다. URL 값은 앞서 복사한 환경 URL을 입력합니다. 환경 URL은 `https://org{{조직 ID}}.crm{{숫자}}.dynamics.com/`와 같은 형식입니다.
   ...
   > **NOTE**: 환경 URL은 반드시 `https://`로 시작합니다.
...
1. 아래 명령어를 통해 현재 로그인된 환경의 솔루션을 확인합니다. `Unique Name` 필드에서 `GPPBParticipant{{숫자}}` 솔루션이 보이면 정상입니다. 이 `Unique Name` 필드의 값을 기억해 두세요.
...
1. 아래 명령어를 통해 솔루션을 내보냅니다.
   ...
   > **NOTE**:
   > 
   > - `--name` 파라미터의 값은 앞서 적어둔 `Unique Name` 필드의 값입니다.
   > - `--overwrite` 파라미터는 이미 내보내진 솔루션을 덮어쓰기 위한 옵션입니다.
...
1. 솔루션 파일의 압축을 해제합니다. 아래 `GPPBParticipant{{숫자}}`는 앞서 적어둔 `Unique Name` 필드의 값입니다.
...
1. 파워 플랫폼 CLI를 통해 다운로드 받은 솔루션 파일을 삭제합니다. 아래 `GPPBParticipant{{숫자}}`는 앞서 적어둔 `Unique Name` 필드의 값입니다.
...
1. 아래 명령어를 하나씩 실행시켜 솔루션 데이터를 압축합니다.

    ```bash
    pushd ./GPPBParticipant{{숫자}}/
    zip -r ./GPPBParticipant{{숫자}}.zip .
    mv ./GPPBParticipant{{숫자}}.zip ../
    popd
    ls -al
    ```
...
1. 흐름을 저장한 후 테스트를 실행해서 워크플로우가 잘 작동하는 것을 확인합니다. 만약 실패한다면, GitHub 코드스페이스의 백엔드 API 앱이 실행 중인지 확인합니다. 실행중이 아니라면, 다시 실행시킵니다.
